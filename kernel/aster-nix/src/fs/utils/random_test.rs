...
impl Operation {
    ...
    pub fn generate_random_file_operation(
        ...
        if op_id == Self::READ_ID {
            let (offset, len) = generate_random_offset_len(Self::MAX_PAGE_PER_FILE * PAGE_SIZE, rng);
    ...
    pub fn execute_and_test(&mut self, op: Operation) {
        match op {
            Operation::Create(name, type_) => {
                self.test_create(&name, type_)
            }
            Operation::Lookup(name) => {
                self.test_lookup(&name)
            }
            Operation::Readdir() => {
                self.test_readdir()
            }
            Operation::Unlink(name) => {
                self.test_unlink(&name)
            }
            Operation::Rmdir(name) => {
                self.test_rmdir(&name)
            }
            Operation::Rename(old_name, new_name) => {
                self.test_rename(&old_name, &new_name)
            }
            _ => {}
        }
    }
...
impl FileInMemory {
    ...
    pub fn execute_and_test(&mut self, op: Operation, rng: &mut dyn RngCore) {
        match op {
            Operation::Read(offset, len) => {
                self.test_read(offset, len)
            }
            Operation::Write(offset, len) => {
                self.test_write(offset, len, rng)
            }
            Operation::Resize(new_size) => {
                self.test_resize(new_size)
            }
            _ => {}
        }
    }
...
pub fn generate_random_operation<'a>(
    ...
    let op = match dentry {
        DentryInMemory::Dir(dir) => {
            Operation::generate_random_dir_operation(dir, idx, rng)
        }
        DentryInMemory::File(file) => {
            Operation::generate_random_file_operation(file, idx, rng)
        }
    };
    (dentry, op)
}
